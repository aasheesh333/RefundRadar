...
class ReminderGenerator {
  ...
  List<Reminder> forDispute(Dispute d) {
    ...
    if (l1Filed != null && d.status != DisputeStatus.resolved && d.status != DisputeStatus.expired) {
      ...
      if (fire.isAfter(now) && d.status == DisputeStatus.filedL1) {
        out.add(_make(
          d,
    ...
    if (l2Filed != null && d.status != DisputeStatus.resolved && d.status != DisputeStatus.expired) {
      ...
      if (fire.isAfter(now) && d.status == DisputeStatus.filedL2) {
        out.add(_make(
          d,
  ...
  Reminder _make(Dispute d, ReminderStage stage, DateTime fire, String title, String body) {
    return Reminder(
      id: '${d.id}_${stage.id}',
      uid: d.uid,
      disputeId: d.id,
